...
files = [file for file in os.listdir() if '2021-11' not in file]
...
def geofence_trajectories(node, dirc):
    gate = gates[node][dirc]
    
    list_lat, list_lng = [], [] # lists to store all gate coordinates for node & dirc
    
    # filter through each gate and append lat, lng coords to lists
    for coords in gate.values():
        for coord in coords.values():
            list_lat.append(coord[0])
            list_lng.append(coord[1])
            
    # specify min-max lat and lng bounds
    min_lat, max_lat = min(list_lat), max(list_lat)
    min_lng, max_lng = min(list_lng), max(list_lng)
    ...
    for file in files:
        ...
        df = df[(df.Latitude.between(min_lat, max_lat)) & (df.Longitude.between(min_lng, max_lng))]
        ...
        trips = list(set.intersection(*gate_trips))
        
        # filter df for trip IDs passing through all gates
        gdf = df.copy()[df.TripID.isin(trips)].compute()
        
        # remove redundant columns
        gdf.drop(['Altitude', 'unixtime', 'accuracy'], axis = 1, inplace = True)
        
        # save file
        output_path = os.path.join(r"C:\Users\pramesh\Wejo\dilemma_zone_modeling\raw_data", str(node) + '_' + dirc, file)
        gdf.to_csv(output_path, index = False)
        
# geofence trajectories for all directions
for node in gates.keys():
    directions = gates[node].keys()
    for dirc in directions:
        print(f"\nProcessing node, direction: {node}, {dirc}")
        geofence_trajectories(node, dirc)